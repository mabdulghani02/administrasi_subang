...
async function loadData(targetPage = null) {
  ...
  try {
    const [sRes, cRes, eRes, cashRes, attRes, advRes, mRes, iRes, wRes] = await Promise.allSettled([
      ...
      db.from('waste_sales').select('*')
    ]);

    DB.sales = sRes.status === 'fulfilled' && sRes.value.data ? sRes.value.data : [];
    DB.counter = cRes.status === 'fulfilled' && cRes.value.data ? cRes.value.data : [];
    DB.expenses = eRes.status === 'fulfilled' && eRes.value.data ? eRes.value.data : [];
    DB.cash = cashRes.status === 'fulfilled' && cashRes.value.data ? cashRes.value.data : [];
    DB.attendance = attRes.status === 'fulfilled' && attRes.value.data ? attRes.value.data : [];
    DB.advances = advRes.status === 'fulfilled' && advRes.value.data ? advRes.value.data : [];
    DB.masterSalary = mRes.status === 'fulfilled' && mRes.value.data ? mRes.value.data : [];
    DB.installments = iRes.status === 'fulfilled' && iRes.value.data ? iRes.value.data : [];
    DB.wasteSales = wRes.status === 'fulfilled' && wRes.value.data ? wRes.value.data : [];
    ...
    for (const [groupName, items] of Object.entries(groupedPasar)) {
      ...
      items.forEach((ex, i) => {
        shPasar.getRow(rowIdx++).values = [
          ...
          Number(ex.qty || 1),
          ex.satuan || 'PCS',
          Number(ex.harga_satuan || ex.nominal),
          Number(ex.nominal)
        ];
...
// Menghitung kelebihan jam kerja per hari (0 hingga 5 jam)
function calculateDailyOvertimeHours(masukStr, pulangStr) {
  const durasi = calculateHours(masukStr, pulangStr);
  if (durasi <= STANDARD_WORK_HOURS) return 0;
  const rawOvertime = durasi - STANDARD_WORK_HOURS;
...
async function exportMonthlyExcel() {
  ...
  try {
    ...
    shPasar.getCell('B5').value = 'TOTAL';
    shPasar.getCell('D5').value = sumPasar;
color: var(--muted);">Pilih Bulan Rekapitulasi:</label>
      <input type="month" id="dashboardMonth" value="${currentMonth}" onchange="updateDashboardMetrics(this.value)" style="padding: 12px; border: 1px solid var(--line); border-radius: 10px; font-size: 16px; width: 100%; background: var(--card); color: var(--text);">
    </div>
  </div>
  <div class="cards">
