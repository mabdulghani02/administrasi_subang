...
function totalESB(data) {
  return Number(data.makanan || 0) + Number(data.minuman || 0) + Number(data.tahu || 0) + Number(data.gorengan || 0) + Number(data.lain_lain || 0) + Number(data.pajak || 0);
}

function totalCounter(data) {
  return Number(data.cash || 0) + Number(data.debit_card || 0) + Number(data.grab || 0) + Number(data.qris || 0);
}
...
function updateDashboardMetrics(yearMonth) {
  ...
  const pctLainnya = totalAllKategori ? 100 - (pctMakanan + pctMinuman + pctTahu + pctGorengan) : 0;
...
async function exportMonthlyExcel() {
  ...
  try {
    ...
    expCikuda.forEach((ex, i) => {
      shCikuda.getRow(rowIdx++).values = [null, i+1, ex.tanggal, ex.sumber, Number(ex.qty|karyawanMgt.push('Karyawan Manajemen ' + (karyawanMgt.length + 1));
  }
...
<button id="fabExport" class="fab-export" onclick="exportMonthlyExcel()"><i class="fa-solid fa-file-excel"></i> Export Bulanan</button>
`;
updateDashboardMetrics(currentMonth); setTimeout(() => { const marker = $('dashboardBottomMarker'); const fab = $('fabExport'); if (marker && fab) { const obs = new IntersectionObserver(entries => { if (entries[0].isIntersecting) { fab.classList.add('show'); } else { fab.classList.remove('show'); }
}, { threshold: 0.1 }); obs.observe(marker); } }, 300); } let monthlyChartInstance = null; function updateDashboardMetrics(yearMonth) { const [targetYear, targetMonth] = yearMonth.split('-'); const monthCounters = (DB.counter || []).filter(r => formatDate(r.tanggal).startsWith(yearMonth));
const monthExpenses = (DB.expenses || []).filter(r => formatDate(r.tanggal).startsWith(yearMonth)); const monthSales = (DB.sales || []).filter(r => formatDate(r.tanggal).startsWith(yearMonth)); $('cardOmset').textContent = money(sum(monthCounters.map(r => totalCounter(r))));
$('cardExpense').textContent = money(sum(monthExpenses.map(r => r.nominal))); let sumMakanan = 0, sumMinuman = 0, sumTahu = 0, sumGorengan = 0, sumLain = 0; monthSales.forEach(s => { sumMakanan += Number(s.makanan || 0); sumMinuman += Number(s.minuman || 0); sumTahu += Number(s.tahu || 0);
sumGorengan += Number(s.gorengan || 0); sumLain += Number(s.lain_lain || 0); }); const totalAllKategori = sumMakanan + sumMinuman + sumTahu + sumGorengan + sumLain; const pctMakanan = totalAllKategori ? Math.round((sumMakanan / totalAllKategori) * 100) : 0;
const pctMinuman = totalAllKategori ? Math.round((sumMinuman / totalAllKategori) * 100) : 0; const pctTahu = totalAllKategori ? Math.round((sumTahu / totalAllKategori) * 100) : 0; const pctGorengan = totalAllKategori ? Math.round((sumGorengan / totalAllKategori) * 100) : 0;
const pctLainnya = totalAllKategori ? 100 - (pctMakanan + pctMinuman + pctTahu + pctGorengan) : 0; // Sisa persentase agar pas 100%
$('totalItemQty').textContent = formatNum(totalAllKategori); $('persenMakanan').textContent = `${pctMakanan}%`; $('persenMinuman').textContent = `${pctMinuman}%`; $('persenTahu').textContent = `${pctTahu}%`; $('persenGorengan').textContent = `${pctGorengan}%`;
$('persenLainnya').textContent = `${pctLainnya}%`; const donutChart = $('kategoriDonut'); if (donutChart) { if (totalAllKategori === 0) { donutChart.style.background = `conic-gradient(var(--line) 0% 100%)`; } else { const stop1 = pctMakanan; const stop2 = stop1 + pctMinuman;
const stop3 = stop2 + pctTahu; const stop4 = stop3 + pctGorengan;
donutChart.style.background = `conic-gradient(
var(--wa-primary) 0% ${stop1}%,
var(--wa-teal) ${stop1}% ${stop2}%,
var(--success) ${stop2}% ${stop3}%,
...
} } const ctxMonthly = document.getElementById('monthlySalesChart'); if (ctxMonthly) { if (monthlyChartInstance) monthlyChartInstance.destroy(); const daysInMonth = new Date(targetYear, targetMonth, 0).getDate(); const labels = []; const dataOmset = []; for (let day = 1; day <= daysInMonth; day++) {
const dayStr = `${yearMonth}-${String(day).padStart(2, '0')}`; labels.push(String(day)); const match = monthCounters.find(r => formatDate(r.tanggal) === dayStr); dataOmset.push(match ? totalCounter(match) : 0); } monthlyChartInstance = new Chart(ctxMonthly, { type: 'line', data: { labels: labels,
datasets: [{ label: 'Omset Harian (Rp)', data: dataOmset, borderColor: '#007aff', backgroundColor: 'rgba(0, 122, 255, 0.15)', fill: true, tension: 0.3 }] }, options: { responsive: true, maintainAspectRatio: false } }); } } async function exportMonthlyExcel() { if (typeof ExcelJS === 'undefined') {
showToast('Library ExcelJS belum dimuat. Tambahkan script CDN di HTML.'); return; } showToast('Sedang merakit file Excel Multi-Sheet...'); try { const yearMonth = $('dashboardMonth').value; const [y, mStr] = yearMonth.split('-');
const mNames = ['JANUARI', 'FEBRUARI', 'MARET', 'APRIL', 'MEI', 'JUNI', 'JULI', 'AGUSTUS', 'SEPTEMBER', 'OKTOBER', 'NOVEMBER', 'DESEMBER']; const monthName = mNames[parseInt(mStr, 10) - 1]; const wb = new ExcelJS.Workbook(); const titleFont = { bold: true, size: 12 };
const headerFont = { bold: true }; const shSum = wb.addWorksheet('SUMMARY'); shSum.getCell('B1').value = `LAPORAN KEUANGAN SARI KEDELE CABANG SUBANG ${monthName} ${y}`; shSum.getCell('B1').font = titleFont; const shOmset = wb.addWorksheet('OMSET');
shOmset.getCell('B2').value = 'OMSET HARIAN UNIT SUBANG'; shOmset.getCell('B2').font = titleFont; shOmset.getRow(3).values = [null, 'No.', 'TANGGAL', 'CASH', 'CARD', 'QRIS', 'GRAB', 'PAJAK']; shOmset.getRow(3).font = headerFont;
const salesFilter = DB.sales.filter(r => formatDate(r.tanggal).startsWith(yearMonth)); salesFilter.sort((a,b) => a.tanggal.localeCompare(b.tanggal)); let rowIdx = 4; let totalCash = 0, totalCard = 0, totalQris = 0, totalGrab = 0, totalPajak = 0; salesFilter.forEach((s, idx) => {
const dateObj = new Date(formatDate(s.tanggal)); shOmset.getRow(rowIdx).values = [ null, idx + 1, dateObj, Number(s.cash||0), Number(s.debit_card||0), Number(s.qris||0), Number(s.grab||0), Number(s.pajak||0) ]; totalCash += Number(s.cash||0); totalCard += Number(s.debit_card||0);
totalQris += Number(s.qris||0); totalGrab += Number(s.grab||0); totalPajak += Number(s.pajak||0); rowIdx++; }); shOmset.getRow(rowIdx).values = [null, '', 'TOTAL', totalCash, totalCard, totalQris, totalGrab, totalPajak]; shOmset.getRow(rowIdx).font = headerFont;
const expFilter = DB.expenses.filter(r => formatDate(r.tanggal).startsWith(yearMonth)); const expPasar = expFilter.filter(r => r.kategori === 'PASAR'); const expCikuda = expFilter.filter(r => r.kategori === 'CIKUDA'); const expSkf = expFilter.filter(r => r.kategori === 'SKF');
const expLain = expFilter.filter(r => r.kategori === 'LAIN-LAIN' || !r.kategori); const shPasar = wb.addWorksheet('PEMBELANJAAN PASAR'); shPasar.getCell('B1').value = 'Lampiran 5'; shPasar.getCell('B3').value = 'PEMBELANJAAN ALAT DAN BAHAN BAKU'; const sumPasar = sum(expPasar.map(r => r.nominal));
shPasar.getCell('B5').value = 'TOTAL'; shPasar.getCell('D5').value = sumPasar; shPasar.getCell('B5').font = headerFont; shPasar.getCell('D5').font = headerFont; rowIdx = 7; const groupedPasar = {}; expPasar.forEach(ex => { const groupName = (ex.sub_kategori || 'LAINNYA').trim().toUpperCase();
if (!groupedPasar[groupName]) groupedPasar[groupName] = []; groupedPasar[groupName].push(ex); }); for (const [groupName, items] of Object.entries(groupedPasar)) { shPasar.getCell(`H${rowIdx}`).value = groupName; shPasar.getCell(`H${rowIdx}`).font = headerFont; rowIdx++;
shPasar.getRow(rowIdx).values = [null, 'NO', 'TANGGAL', 'JENIS BARANG', 'QTY', 'SATUAN', 'HARGA SATUAN', 'JUMLAH (Rp)']; shPasar.getRow(rowIdx).font = headerFont; rowIdx++; let subtotalGroup = 0; items.forEach((ex, i) => {
shPasar.getRow(rowIdx++).values = [null, i+1, ex.tanggal, ex.sumber, Number(ex.qty||1), ex.satuan||'PCS', Number(ex.harga_satuan||ex.nominal), Number(ex.nominal)]; subtotalGroup += Number(ex.nominal); }); shPasar.getCell(`H${rowIdx}`).value = subtotalGroup;
shPasar.getCell(`H${rowIdx}`).font = headerFont; rowIdx += 2; } const shCikuda = wb.addWorksheet('PEMBELANJAAN CIKUDA'); shCikuda.getCell('B1').value = 'Lampiran 6'; shCikuda.getRow(5).values = [null, 'No', 'Tanggal', 'Nama Barang', 'QTT', 'Harga', 'Total']; shCikuda.getRow(5).font = headerFont;
rowIdx = 6; let sumCikuda = 0; expCikuda.forEach((ex, i) => { shCikuda.getRow(rowIdx++).values = [null, i+1, ex.tanggal, ex.sumber, Number(ex.qty||1), Number(ex.harga_satuan||ex.nominal), Number(ex.nominal)]; sumCikuda += Number(ex.nominal); }); const shSkf = wb.addWorksheet('PEMBELANJAN SKF');
shSkf.getCell('B3').value = 'Pembelanjaan dari SKF'; shSkf.getRow(5).values = [null, 'NO', 'TANGGAL', 'KETERANGAN', 'QTY', 'JUMLAH SATUAN', 'TOTAL']; shSkf.getRow(5).font = headerFont; rowIdx = 6; let sumSkf = 0; expSkf.forEach((ex, i) => {
shSkf.getRow(rowIdx++).values = [null, i+1, ex.tanggal, ex.sumber, Number(ex.qty||1), Number(ex.harga_satuan||ex.nominal), Number(ex.nominal)]; sumSkf += Number(ex.nominal); }); const shLain = wb.addWorksheet('LAIN-LAIN'); shLain.getCell('C1').value = 'Lampiran 8';
shLain.getCell('C2').value = 'PENGELUARAN ALAT DAN LAIN-LAIN'; shLain.getCell('C3').value = 'TOTAL'; const sumLain = sum(expLain.map(r => r.nominal)); shLain.getCell('H3').value = sumLain;
shLain.getRow(4).values = [null, null, null, 'NO', 'TANGGAL', 'JENIS BARANG', 'QTY', 'SATUAN', 'HARGA SATUAN', 'JUMLAH (Rp)']; shLain.getRow(4).font = headerFont; rowIdx = 5; expLain.forEach((ex, i) => {
shLain.getRow(rowIdx++).values = [null, null, null, i+1, ex.tanggal, ex.sumber, Number(ex.qty||1), ex.satuan||'PCS', Number(ex.harga_satuan||ex.nominal), Number(ex.nominal)]; }); const shGaji = wb.addWorksheet('GAJI KARYAWAN'); shGaji.getCell('B1').value = 'Lampiran 7';
shGaji.getCell('B2').value = 'GAJI KARYAWAN'; shGaji.getCell('B3').value = 'TOTAL GAJI KARYAWAN'; const lastD = new Date(y, parseInt(mStr, 10), 0).getDate(); const sDate = `${yearMonth}-01`; const eDate = `${yearMonth}-${String(lastD).padStart(2, '0')}`;
const payroll = getCalculatedPayrollList(sDate, eDate, 'ALL'); const sumGaji = sum(payroll.map(p => p.gajiBersih)); shGaji.getCell('E3').value = sumGaji; shGaji.getCell('E3').font = headerFont; shGaji.getCell('B5').value = 'GAJI BULANAN';
shGaji.getRow(6).values = [null, 'NO', 'Nama Pegawai', 'GAJI POKOK', 'Tunjangan', null, null, null, null, null, null, 'TOTAL GAJI', 'Potongan', null, null, 'Gaji Bersih']; shGaji.getRow(6).font = headerFont; rowIdx = 7; payroll.forEach((p, i) => {
const tunjangan = p.jabatan + p.prestasi + p.kesehatan + p.jamKerja + p.zakat + p.loyalitas + p.lainLain; const potongan = p.kasbon + p.bpjs + p.cicilan;
shGaji.getRow(rowIdx++).values = [null, i+1, p.nama, p.pokok, tunjangan, null, null, null, null, null, null, p.pokok + tunjangan, potongan, null, null, p.gajiBersih]; }); rowIdx += 2; shGaji.getCell(`B${rowIdx}`).value = 'REKAP UANG JAJAN (PER TANGGAL)';
shGaji.getCell(`B${rowIdx}`).font = headerFont; rowIdx++; shGaji.getRow(rowIdx).values = [null, 'NO', 'TANGGAL', 'TOTAL UANG JAJAN']; shGaji.getRow(rowIdx).font = headerFont; rowIdx++; const attFilter = DB.attendance.filter(r => formatDate(r.tanggal).startsWith(yearMonth));
const groupedUangJajan = {}; attFilter.forEach(att => { if (att.status === 'Hadir') { const shift = classifyShift(att.masuk); if (shift.onTime) { const tgl = formatDate(att.tanggal); if (!groupedUangJajan[tgl]) groupedUangJajan[tgl] = 0; groupedUangJajan[tgl] += DEFAULT_ALLOWANCE; } } });
const sortedDates = Object.keys(groupedUangJajan).sort(); let sumUangJajanTotal = 0; let ujNo = 1; sortedDates.forEach(tgl => { const nominal = groupedUangJajan[tgl]; shGaji.getRow(rowIdx++).values = [null, ujNo++, tgl, nominal]; sumUangJajanTotal += nominal; });
shGaji.getRow(rowIdx).values = [null, '', 'TOTAL', sumUangJajanTotal]; shGaji.getRow(rowIdx).font = headerFont; shSum.getCell('B3').value = 'TOTAL OMSET'; shSum.getCell('C3').value = totalCash + totalCard + totalQris + totalGrab; shSum.getCell('B4').value = 'PENGELUARAN PASAR';
shSum.getCell('C4').value = sumPasar; shSum.getCell('B5').value = 'PENGELUARAN CIKUDA'; shSum.getCell('C5').value = sumCikuda; shSum.getCell('B6').value = 'PENGELUARAN SKF'; shSum.getCell('C6').value = sumSkf; shSum.getCell('B7').value = 'PENGELUARAN LAIN-LAIN'; shSum.getCell('C7').value = sumLain;
shSum.getCell('B8').value = 'GAJI KARYAWAN'; shSum.getCell('C8').value = sumGaji; shSum.getCell('B9').value = 'UANG JAJAN ABSENSI'; shSum.getCell('C9').value = sumUangJajanTotal; const totalPengeluaranAll = sumPasar + sumCikuda + sumSkf + sumLain + sumGaji + sumUangJajanTotal;
shSum.getCell('B11').value = 'SISA/PROFIT'; shSum.getCell('C11').value = (totalCash + totalCard + totalQris + totalGrab) - totalPengeluaranAll; const buffer = await wb.xlsx.writeBuffer(); const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = `LAPORAN_KEUANGAN_SUBANG_${monthName}_${y}.xlsx`; link.click(); showToast('Excel berhasil diunduh!'); } catch (err) { console.error(err); showToast('Gagal membuat Excel: ' + err.message); } }
...
<div id="salesSubContent"></div>
`;
showSalesSub('input'); } function showSalesSub(type) { const container = $('salesSubContent'); if (!container) return; const btnIn = $('subBtnInput'); const btnRep = $('subBtnReport'); if (btnIn && btnRep) { btnIn.classList.toggle('active-sub', type === 'input');
btnRep.classList.toggle('active-sub', type === 'report'); } if (type === 'input') {
container.innerHTML = `
<div class="panel">
<div class="panel-title">PENDAPATAN ESB</div>
<form id="salesForm">
...
`;
$('salesForm').onsubmit = async e => { e.preventDefault(); const formData = Object.fromEntries(new FormData(e.target)); const { error } = await db.from('sales').upsert([formData], { onConflict: 'tanggal' }); if (error) showToast('Gagal: ' + error.message);
else { showToast('Laporan ESB disimpan.'); loadData('sales'); } }; $('counterForm').onsubmit = async e => { e.preventDefault(); const formData = Object.fromEntries(new FormData(e.target)); const { error } = await db.from('counter').upsert([formData], { onConflict: 'tanggal' });
if (error) showToast('Gagal: ' + error.message); else { showToast('Laporan Konter disimpan.'); loadData('sales'); } }; } else {
container.innerHTML = `
<div class="panel">
<div style="display:flex; flex-direction:column; gap:10px; margin-bottom:14px;">
<label style="font-weight:700;">Pilih Tanggal Laporan:</label>
...
`;
loadReport(); } } function inputField(name, label, value = '', type = 'number') {
return `
<div class="field">
<label>${label}</label>
<input name="${name}" type="${type}" value="${value}" ${type === 'number' ? 'min="0" step="1"' : ''}>
...
`;
} function reportRow(label, value, forceDash = false) { const number = Number(value || 0); const isNegative = number < 0; const displayValue = Math.abs(number).toLocaleString('id-ID'); const display = forceDash || number === 0 ? '-' : (isNegative ? '-' + displayValue : displayValue);
return `
<div style="display: flex; justify-content: space-between; margin-bottom: 4px; font-size: 13.5px;">
<span style="flex: 1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${label}</span>
<span style="white-space: nowrap;">&nbsp;&nbsp;&nbsp;: Rp</span>
...
`;
} function loadReport() { const dateEl = $('reportDate'); const resultEl = $('reportResult'); if (!dateEl) { console.error('Elemen #reportDate tidak ditemukan'); return; } if (!resultEl) { console.error('Elemen #reportResult tidak ditemukan'); return; } try {
const selectedDate = normalizeDate(dateEl.value); const salesFound = (DB.sales || []).find(r => normalizeDate(r.tanggal) === selectedDate); const counterFound = (DB.counter || []).find(r => normalizeDate(r.tanggal) === selectedDate); const sales = salesFound || {};
const counter = counterFound || {}; const totalEsb = totalESB(sales); const totalKonterVal = totalCounter(counter); const diffCash = Number(sales.cash || 0) - Number(counter.cash || 0); const diffDebit = Number(sales.debit_card || 0) - Number(counter.debit_card || 0);
const diffGrab = Number(sales.grab || 0) - Number(counter.grab || 0); const diffQris = Number(sales.qris || 0) - Number(counter.qris || 0); const dateObject = new Date(selectedDate + 'T00:00:00'); const formattedDate = isNaN(dateObject) ? selectedDate : dateObject.toLocaleDateString('id-ID', {
weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }); const statusInfo = (!salesFound && !counterFound) ? `<div style="text-align:center; padding: 10px; color: #dc2626; font-weight: bold; margin-bottom: 10px;">Belum ada data penjualan pada tanggal ini.</div>` : '';
resultEl.innerHTML = `
<div class="report-container">
${statusInfo}
<div id="captureDailyReport" style="padding: 20px; background: white; color: black; border-radius: 8px; border: 1px solid #e2e8f0; font-family: Arial, sans-serif;">
...
`;
} } function downloadDailyReportImage() { const target = $('captureDailyReport'); if (!target) { showToast('Laporan belum dimuat atau kosong.'); return; } const date = $('reportDate').value || today(); downloadElementAsImage('captureDailyReport', 'LAPORAN_PENDAPATAN_' + date); }
...
`;
showExpenseSub('input'); } function showExpenseSub(type) { const container = $('expenseSubContent'); if (!container) return; const btnIn = $('expBtnIn'); const btnRep = $('expBtnRep'); const btnCash = $('expBtnCash'); if (btnIn && btnRep && btnCash) {
btnIn.classList.toggle('active-sub', type === 'input'); btnRep.classList.toggle('active-sub', type === 'report'); btnCash.classList.toggle('active-sub', type === 'cash'); } if (type === 'input') {
container.innerHTML = `
<div class="panel">
...
`;
for (let i = 0; i < 3; i++) addExpenseRow(); } else if (type === 'report') {
container.innerHTML = `
<div class="panel">
<div style="display:flex; flex-direction:column; gap:10px; margin-bottom:14px;">
<label style="font-weight:700;">Pilih Tanggal Laporan:</label>
...
`;
loadExpenseReport(); } else { const currentDate = today(); const cash = (DB.cash || []).find(c => formatDate(c.tanggal) === currentDate) || {};
container.innerHTML = `
<div class="panel">
...
`;
$('cashForm').onsubmit = async e => { e.preventDefault(); const formData = Object.fromEntries(new FormData(e.target)); const payload = { tanggal: formData.tanggal, saldo_harian: Number(formData.saldo_harian || 0), belanja_malam: Number(formData.belanja_malam || 0) };
const { error } = await db.from('cash_positions').upsert([payload], { onConflict: 'tanggal' }); if (error) showToast('Gagal: ' + error.message); else { showToast('Posisi kas disimpan.'); loadData('expense'); } }; } } function addExpenseRow() { const tr = document.createElement('tr');
...
`;
$('batchExpenseBody').appendChild(tr); } async function submitBatchExpenses() { const tanggal = $('batchExpenseDate').value; const items = []; document.querySelectorAll('.expense-input-row').forEach(r => { const cat = r.querySelector('.exp-kategori').value;
const sub = r.querySelector('.exp-sub').value.trim(); const s = r.querySelector('.exp-sumber').value.trim(); const q = r.querySelector('.exp-qty').value; const sat = r.querySelector('.exp-satuan').value.trim(); const h = r.querySelector('.exp-harga').value;
const n = r.querySelector('.exp-nominal').value; if (s || n) items.push({ tanggal, kategori: cat, sub_kategori: sub, sumber: s, qty: Number(q || 1), satuan: sat || 'PCS', harga_satuan: Number(h || 0), nominal: Number(n || 0), keterangan: '-' }); });
if (!items.length) { showToast('Isi minimal satu baris.'); return; } const { error } = await db.from('expenses').insert(items); if (error) showToast('Gagal: ' + error.message); else { showToast('Pengeluaran berhasil disimpan.'); loadData('expense'); } } function loadExpenseReport() {
const dateEl = $('expenseReportDate'); const resultEl = $('expenseReportResult'); if (!dateEl) { console.error('Elemen #expenseReportDate tidak ditemukan'); return; } if (!resultEl) { console.error('Elemen #expenseReportResult tidak ditemukan'); return; } try {
const date = normalizeDate(dateEl.value); const cashRow = DB.cash.find(r => formatDate(r.tanggal) === date) || {}; const dayExpenses = DB.expenses.filter(r => formatDate(r.tanggal) === date); const totalPemasukan = Number(cashRow.saldo_harian || 0) + Number(cashRow.belanja_malam || 0);
const totalPengeluaran = sum(dayExpenses.map(r => r.nominal)); const sisaSaldo = totalPemasukan - totalPengeluaran; const dateObject = new Date(date + 'T00:00:00'); const formattedDate = isNaN(dateObject) ? date : dateObject.toLocaleDateString('id-ID', {
weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }); let expenseRowsHtml = dayExpenses.length > 0 ? dayExpenses.map((expense, i) => reportRow(`${i + 1}. ${escapeHtml(expense.sumber || 'Pengeluaran')}`, expense.nominal)).join('')
...
`;
} catch (error) { console.error('Gagal memuat laporan pengeluaran:', error);
resultEl.innerHTML = `
<div style="color:#dc2626; padding: 15px; border: 1px solid #dc2626; border-radius: 8px;">
Gagal memuat laporan pengeluaran. <br><small>${error.message}</small>
</div>
`;
} } function downloadExpenseReportImage() { const target = $('captureExpenseReport'); if (!target) { showToast('Laporan pengeluaran belum dimuat.'); return; } const date = $('expenseReportDate').value || today(); downloadElementAsImage('captureExpenseReport', 'LAPORAN_PENGELUARAN_' + date); }
...
`;
showAttendanceSub('log'); } function showAttendanceSub(type) { const container = $('attendanceSubContent'); if (!container) return; const btnLog = $('attBtnLog'); const btnAllow = $('attBtnAllow'); const btnHours = $('attBtnHours'); if (btnLog && btnAllow && btnHours) {
btnLog.classList.toggle('active-sub', type === 'log'); btnAllow.classList.toggle('active-sub', type === 'allowance'); btnHours.classList.toggle('active-sub', type === 'hours'); } if (type === 'log') {
container.innerHTML = `
<div class="panel">
...
`;
renderAttendanceTable(); } else if (type === 'allowance') {
container.innerHTML = `
<div class="panel">
<div style="display:flex; flex-direction: column; gap: 8px; margin-bottom:14px;">
...
`;
renderWorkHoursTable(); } } function renderAttendanceTable() { const fDate = formatDate($('attendanceFilterDate')?.value) || today(); const list = (DB.attendance || []).filter(r => formatDate(r.tanggal) === fDate); const tbody = $('attendanceTableBody'); if (!tbody) return; if (!list.length) {
tbody.innerHTML = `<tr><td colspan="6" class="empty">Belum ada absensi tanggal ${fDate}.</td></tr>`; return; }
...
`).join('');
} function renderAllowanceTable() { const fDate = formatDate($('allowanceFilterDate')?.value) || today(); const allAttToday = (DB.attendance || []).filter(r => formatDate(r.tanggal) === fDate); const tbody = $('allowanceTableBody'); if (!tbody) return; if (!allAttToday.length) {
tbody.innerHTML = `<tr><td colspan="8" class="empty">Belum ada absensi tanggal ${fDate}.</td></tr>`; return; } const list = allAttToday.map(r => ({ ...r, c: classifyShift(r.masuk) })).filter(r => r.c.onTime); if (!list.length) {
tbody.innerHTML = `<tr><td colspan="8" class="empty">Tidak ada yang tepat waktu.</td></tr>`; return; }
tbody.innerHTML = list.map((r, i) => `
<tr style="border-bottom: 1px solid var(--line);">
<td class="center" style="color:var(--muted);">${i+1}</td>
<td style="font-weight:700;">${escapeHtml(r.nama)}</td>
...
`).join('');
} function renderWorkHoursTable() { const fDate = formatDate($('workHoursFilterDate')?.value) || today(); const list = (DB.attendance || []).filter(r => formatDate(r.tanggal) === fDate); const tbody = $('workHoursTableBody'); if (!tbody) return;
if (!list.length) { tbody.innerHTML = `<tr><td colspan="8" class="empty">Belum ada data.</td></tr>`; return; } tbody.innerHTML = list.map((r, i) => { const durasi = calculateHours(r.masuk, r.pulang); let diff = 0; let roundedDiff = 0; let nominal = 0; if (durasi > 0) { diff = durasi - 11;
roundedDiff = Math.round(diff); nominal = roundedDiff * 5000; }
return `
<tr style="border-bottom: 1px solid var(--line);">
<td class="center" style="color:var(--muted);">${i+1}</td>
<td style="font-weight:700;">${escapeHtml(r.nama)}</td>
...
`;
}).join(''); } function handleExcelUpload(event) { const file = event.target.files[0]; if (!file) return; $('uploadProgress').style.display = 'block'; const reader = new FileReader(); reader.onload = async function(e) { try { const data = new Uint8Array(e.target.result);
const workbook = XLSX.read(data, { type: 'array' }); const sheetName = workbook.SheetNames.find(s => s.toLowerCase() === 'log') || workbook.SheetNames[0]; const worksheet = workbook.Sheets[sheetName]; const rows = XLSX.utils.sheet_to_json(worksheet, { header: 1, defval: '' }); let periodText = '';
for (let r = 0; r < Math.min(5, rows.length); r++) { for (let c = 0; c < rows[r].length; c++) { if (String(rows[r][c]).includes('~')) { periodText = String(rows[r][c]); break; } } if (periodText) break; } let startYear = new Date().getFullYear(); let startMonth = new Date().getMonth() + 1;
let startDay = 1; const periodMatch = periodText.match(/(\d{4})[/-](\d{2})[/-](\d{2})/); if (periodMatch) { startYear = parseInt(periodMatch[1], 10); startMonth = parseInt(periodMatch[2], 10); startDay = parseInt(periodMatch[3], 10); } const parsedRecords = [];
for (let i = 0; i < rows.length; i++) { const rowStr = rows[i].join(' '); if (rowStr.includes('No :') && rowStr.includes('Nama :')) { const daysRow = i > 0 ? rows[i - 1] : []; const infoRow = rows[i]; const punchRow = i + 1 < rows.length ? rows[i + 1] : []; let namaVal = '', deptVal = '';
for (let c = 0; c < infoRow.length; c++) { const cellStr = String(infoRow[c]).trim(); if (cellStr.includes('Nama :')) { for (let k = c + 1; k < infoRow.length; k++) { if (String(infoRow[k]).trim()) { namaVal = String(infoRow[k]).trim(); break; } } } else if (cellStr.includes('Dept :')) {
for (let k = c + 1; k < infoRow.length; k++) { if (String(infoRow[k]).trim()) { deptVal = String(infoRow[k]).trim(); break; } } } } if (!namaVal) continue; for (let col = 0; col < daysRow.length; col++) { const dayNum = parseInt(daysRow[col], 10);
if (isNaN(dayNum) || dayNum < 1 || dayNum > 31) continue; let currentMonth = startMonth; let currentYear = startYear; if (startDay > 15 && dayNum < 15) { currentMonth++; if (currentMonth > 12) { currentMonth = 1; currentYear++; } }
const dateStr = `${currentYear}-${String(currentMonth).padStart(2, '0')}-${String(dayNum).padStart(2, '0')}`; const punchStr = String(punchRow[col] || '').trim(); const times = punchStr.split(/[\n\r]+/).map(t => t.trim()).filter(t => t.includes(':')); let masukVal = ''; let pulangVal = '';
if (times.length > 0) { masukVal = times[0]; } if (times.length > 1) { pulangVal = times[times.length - 1]; } parsedRecords.push({ tanggal: dateStr, nama: namaVal, departemen: deptVal, masuk: masukVal, pulang: pulangVal, status: times.length > 0 ? 'Hadir' : 'Tidak Hadir' }); } i += 1; } }
const { error } = await db.from('attendance').upsert(parsedRecords, { onConflict: 'tanggal,nama' }); $('uploadProgress').style.display = 'none'; if (error) showToast('Gagal: ' + error.message); else { showToast('Absensi berhasil diunggah.'); loadData('attendance'); } } catch (err) {
$('uploadProgress').style.display = 'none'; showToast('Gagal memproses file: ' + err.message); } }; reader.readAsArrayBuffer(file); } function renderPayrollPage() {
$('content').innerHTML = `
<div class="top">
...
`;
showPayrollSub('rekap'); } function showPayrollSub(type) { const container = $('payrollSubContent'); if (!container) return; const btnRekap = $('payBtnRekap'); const btnSlip = $('payBtnSlip'); const btnKasbon = $('payBtnKasbon'); const btnCicilan = $('payBtnCicilan');
if (btnRekap && btnSlip && btnKasbon && btnCicilan) { btnRekap.classList.toggle('active-sub', type === 'rekap'); btnSlip.classList.toggle('active-sub', type === 'slips'); btnKasbon.classList.toggle('active-sub', type === 'kasbon'); btnCicilan.classList.toggle('active-sub', type === 'cicilan'); }
const allDepts = Array.from(new Set((DB.masterSalary || []).map(r => String(r.departemen || '').trim()))).sort(); const deptOptionsHtml = allDepts.map(d => `<option value="${escapeHtml(d)}">${escapeHtml(d)}</option>`).join(''); const d = new Date(); const y = d.getFullYear();
const m = String(d.getMonth() + 1).padStart(2, '0'); const lastD = new Date(y, d.getMonth() + 1, 0).getDate(); const todayFormatted = d.toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' }); let defaultStart = `${y}-${m}-01`;
let defaultEnd = `${y}-${m}-${String(lastD).padStart(2, '0')}`; if (type === 'rekap') {
container.innerHTML = `
<div class="panel">
...
`;
renderPayrollCards(); } else if (type === 'slips') {
container.innerHTML = `
<div class="panel">
<div style="display: flex; flex-direction: column; gap: 12px; width: 100%;">
<div>
...
`;
renderSlipPages(); } else if (type === 'kasbon') { const empOptionsHtml = (DB.masterSalary || []).map(d => `<option value="${escapeHtml(d.nama)}">${escapeHtml(d.nama)}</option>`).join(''); const currentMonth = new Date().toISOString().slice(0, 7);
container.innerHTML = `
<div class="panel">
...
`;
$('kasbonForm').onsubmit = async e => { e.preventDefault(); const formData = Object.fromEntries(new FormData(e.target)); const payload = { tanggal: formData.tanggal, nama: formData.nama, nominal: Number(formData.nominal || 0), keterangan: formData.keterangan || '-' };
if (!payload.nominal) { showToast('Nominal tidak boleh nol.'); return; } const { error } = await db.from('advances').insert([payload]); if (error) showToast('Gagal: ' + error.message); else { showToast('Kasbon berhasil dicatat.'); loadData('payroll'); } }; renderKasbonTable();
} else if (type === 'cicilan') { const empOptionsHtml = (DB.masterSalary || []).map(d => `<option value="${escapeHtml(d.nama)}">${escapeHtml(d.nama)}</option>`).join(''); const currentMonth = new Date().toISOString().slice(0, 7);
container.innerHTML = `
<div class="panel">
...
`;
$('cicilanForm').onsubmit = async e => { e.preventDefault(); const formData = Object.fromEntries(new FormData(e.target)); const payload = { tanggal: formData.tanggal, nama: formData.nama, nominal: Number(formData.nominal || 0), tenor: Number(formData.tenor || 1),
keterangan: formData.keterangan || '-' }; if (!payload.nominal) { showToast('Nominal tidak boleh nol.'); return; } const { error } = await db.from('installments').insert([payload]); if (error) showToast('Gagal: ' + error.message); else { showToast('Cicilan berhasil dicatat.'); loadData('payroll'); }
}; renderCicilanTable(); } } function renderKasbonTable() { const fMonth = $('kasbonFilterMonth')?.value || new Date().toISOString().slice(0, 7); const tbody = $('kasbonTableBody'); if (!tbody) return; const list = (DB.advances || []) .filter(r => formatDate(r.tanggal).startsWith(fMonth))
.sort((a,b) => a.tanggal.localeCompare(b.tanggal)); if (!list.length) { tbody.innerHTML = `<tr><td colspan="5" class="empty">Tidak ada data kasbon bulan ini.</td></tr>`; return; }
tbody.innerHTML = list.map((r, i) => `
<tr>
<td class="center">${i+1}</td>
...
`).join('');
} function renderCicilanTable() { const fMonth = $('cicilanFilterMonth')?.value || new Date().toISOString().slice(0, 7); const tbody = $('cicilanTableBody'); if (!tbody) return; const list = (DB.installments || []) .filter(r => formatDate(r.tanggal).startsWith(fMonth))
.sort((a,b) => a.tanggal.localeCompare(b.tanggal)); if (!list.length) { tbody.innerHTML = `<tr><td colspan="7" class="empty">Tidak ada data cicilan bulan ini.</td></tr>`; return; } tbody.innerHTML = list.map((r, i) => { const tenor = Number(r.tenor) || 1;
...
`;
}).join(''); } function getCalculatedPayrollList(sDate, eDate, fDept) { let masterList = DB.masterSalary || []; if (fDept !== 'ALL') { masterList = masterList.filter(r => String(r.departemen).trim().toUpperCase() === fDept.toUpperCase()); }
masterList.sort((a, b) => String(a.nama).localeCompare(String(b.nama))); const eDateObj = new Date(eDate || today()); const eYear = eDateObj.getFullYear(); const eMonth = eDateObj.getMonth(); return masterList.map(emp => { const nmKey = String(emp.nama || '').trim().toUpperCase();
const lainLain = (nmKey === "ZAENAL ARIFIN") ? DEFAULT_BONUS_LAIN : 0; const empAdvances = (DB.advances || []).filter(adv => { const advDate = formatDate(adv.tanggal); return String(adv.nama || '').trim().toUpperCase() === nmKey && advDate >= sDate && advDate <= eDate; });
const kasbonPeriode = sum(empAdvances.map(a => a.nominal)); const empInstallments = (DB.installments || []).filter(ins => String(ins.nama || '').trim().toUpperCase() === nmKey); let cicilanPeriode = 0; empInstallments.forEach(ins => { const insDate = new Date(formatDate(ins.tanggal));
const iYear = insDate.getFullYear(); const iMonth = insDate.getMonth(); const tenor = Number(ins.tenor) || 1; const nominal = Number(ins.nominal) || 0; const perBulan = Math.round(nominal / tenor); const monthDiff = (eYear - iYear) * 12 + (eMonth - iMonth); if (monthDiff >= 0 && monthDiff < tenor) {
cicilanPeriode += perBulan; } }); const empAttendance = (DB.attendance || []).filter(att => { const attDate = formatDate(att.tanggal); return String(att.nama || '').trim().toUpperCase() === nmKey && attDate >= sDate && attDate <= eDate; }); let totalPenyesuaianJam = 0; empAttendance.forEach(att => {
if (att.status === 'Hadir') { const durasi = calculateHours(att.masuk, att.pulang); if (durasi > 0) { const diff = durasi - STANDARD_WORK_HOURS; const roundedDiff = Math.round(diff); totalPenyesuaianJam += (roundedDiff * RATE_PER_HOUR); } } }); const gajiPokok = Number(emp.gaji_pokok || 0);
const jabatan = Number(emp.jabatan || 0); const prestasi = Number(emp.prestasi || 0); const kesehatan = Number(emp.kesehatan || 0); const zakat = Number(emp.zakat || 0); const loyalitas = Number(emp.kebersihan_loyalitas || 0);
const totalPendapatan = gajiPokok + jabatan + prestasi + kesehatan + zakat + loyalitas + lainLain + totalPenyesuaianJam; const totalPotongan = cicilanPeriode + kasbonPeriode; const gajiBersih = Math.max(0, totalPendapatan - totalPotongan); return { nama: emp.nama, departemen: emp.departemen,
pokok: gajiPokok, jabatan: jabatan, prestasi: prestasi, kesehatan: kesehatan, jamKerja: totalPenyesuaianJam, zakat: zakat, loyalitas: loyalitas, lainLain: lainLain, kasbon: kasbonPeriode, bpjs: 0, cicilan: cicilanPeriode, gajiBersih: gajiBersih }; }); } function renderPayrollCards() {
const sDate = $('payrollStartDate')?.value; const eDate = $('payrollEndDate')?.value; const fDept = $('payrollFilterDept')?.value || 'ALL'; const container = $('payrollCardsContainer'); if (!container) return; const list = getCalculatedPayrollList(sDate, eDate, fDept); if (!list.length) {
container.innerHTML = `<div class="panel" style="text-align: center; color: var(--muted);">Belum ada data gaji.</div>`; return; } container.innerHTML = list.map((emp, idx) => {
const totalPendapatan = emp.pokok + emp.jabatan + emp.prestasi + emp.kesehatan + emp.jamKerja + emp.zakat + emp.loyalitas + emp.lainLain; const totalPotongan = emp.kasbon + emp.bpjs + emp.cicilan;
return `
...
`;
}).join(''); } function renderSlipPages() { const sDate = $('slipStartDate')?.value; const eDate = $('slipEndDate')?.value; const fDept = $('slipFilterDept')?.value || 'ALL'; const printDate = $('slipPrintDate')?.value || `Subang, ${today()}`; const container = $('slipPrintContainer');
if (!container) return; const list = getCalculatedPayrollList(sDate, eDate, fDept); if (!list.length) { container.innerHTML = `<div style="text-align: center; padding: 30px; color: var(--muted); background: white;">Tidak ada data.</div>`; return; } let periodMonth = ''; if (sDate) {
const pDate = new Date(sDate); periodMonth = pDate.toLocaleDateString('id-ID', { month: 'long', year: 'numeric' }).toUpperCase(); } let html = ''; list.forEach(emp => {
const totalPendapatan = emp.pokok + emp.jabatan + emp.prestasi + emp.kesehatan + emp.jamKerja + emp.zakat + emp.loyalitas + emp.lainLain; const totalPotongan = emp.kasbon + emp.bpjs + emp.cicilan;
html += `
<div class="slip-card">
<div>
...
`;
}); container.innerHTML = html; } async function exportSlipsToPDF() { const container = $('slipPrintContainer'); if (!container || container.innerHTML.trim() === '') { showToast('Tidak ada slip gaji untuk diekspor.'); return; } showToast('Sedang merakit dokumen PDF...'); try {
const { jsPDF } = window.jspdf; const cards = container.querySelectorAll('.slip-card'); if (!cards.length) { showToast('Slip gaji kosong.'); return; } const pdf = new jsPDF('p', 'mm', 'a4'); for (let i = 0; i < cards.length; i++) { const card = cards[i];
const canvas = await html2canvas(card, { scale: 2, useCORS: true, backgroundColor: '#ffffff' }); const imgData = canvas.toDataURL('image/png'); const imgWidth = 90; const imgHeight = (canvas.height * imgWidth) / canvas.width; const col = i % 2; const row = Math.floor((i / 2) % 3);
const marginX = 12; const marginY = 12; const gapX = 6; const gapY = 6; const posX = marginX + col * (imgWidth + gapX); const posY = marginY + row * (imgHeight + gapY); if (i > 0 && i % 6 === 0) { pdf.addPage(); } pdf.addImage(imgData, 'PNG', posX, posY, imgWidth, imgHeight); }
const sDate = $('slipStartDate')?.value || 'periode'; pdf.save(`SLIP_GAJI_${sDate}.pdf`); showToast('PDF berhasil diunduh!'); } catch (err) { console.error(err); showToast('Gagal membuat PDF: ' + err.message); } } function renderLimbahPage() { const ampasBelumDibagi = (DB.wasteSales || [])
.filter(r => r.jenis === 'Ampas Tahu' && r.keterangan === 'Belum Dibagikan') .reduce((sum, r) => sum + Number(r.nominal), 0); const jalantahBelumDibagi = (DB.wasteSales || []) .filter(r => r.jenis === 'Jalantah' && r.keterangan === 'Belum Dibagikan') .reduce((sum, r) => sum + Number(r.nominal), 0);
$('content').innerHTML = `
<div class="top">
...
`;
renderRincianPembagian('Ampas Tahu', ampasBelumDibagi, 'resultAmpasTahu'); renderWasteSalesTable(); } window.renderRincianPembagian = function(jenis, total, containerId) { const resultEl = $(containerId); if (!resultEl) return; if (total <= 0) {
resultEl.innerHTML = `
...
return; } const perusahaan = total * 0.50; const manajemen = total * 0.25; const pabrik = total * 0.25; const jatahManajemenPerOrang = manajemen / 3; let karyawanMgt = (DB.masterSalary || []) .filter(emp => String(emp.departemen).toLowerCase().includes('manajemen')) .map(emp => emp.nama);
while (karyawanMgt.length < 3) { karyawanMgt.push('Karyawan Manajemen ' + (karyawanMgt.length + 1)); } const tigaManajemen = karyawanMgt.slice(0, 3); let karyawanPabrik = (DB.masterSalary || []) .filter(emp => { const dept = String(emp.departemen).toLowerCase();
return dept.includes('pabrik') || dept.includes('produksi'); }) .map(emp => emp.nama); if (karyawanPabrik.length < 3) { const nonMgt = (DB.masterSalary || []) .filter(emp => !String(emp.departemen).toLowerCase().includes('manajemen')) .map(emp => emp.nama);
karyawanPabrik = [...new Set([...karyawanPabrik, ...nonMgt])]; } const indexZaenal = karyawanPabrik.findIndex(nama => nama.toUpperCase().includes('ZAENAL'));
if (indexZaenal > 0) { // Jika Zaenal ada tapi bukan di urutan pertama
const namaZaenal = karyawanPabrik.splice(indexZaenal, 1)[0]; karyawanPabrik.unshift(namaZaenal); } while (karyawanPabrik.length < 3) { karyawanPabrik.push('Karyawan Pabrik ' + (karyawanPabrik.length + 1)); } const tigaPabrik = karyawanPabrik.slice(0, 3); const jatahPabrik1 = pabrik * 0.40;
const jatahPabrik2 = pabrik * 0.30; const jatahPabrik3 = pabrik * 0.30;
resultEl.innerHTML = `
<div style="border: 1px solid var(--line); border-radius: 8px; padding: 15px; background: var(--card);">
<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px;">
<div>
...
`;
}; window.submitLimbah = async function(jenis, idTgl, idNominal) { const tglEl = $(idTgl); const nomEl = $(idNominal); if (!tglEl || !nomEl) return; const tanggal = tglEl.value; const nominal = Number(nomEl.value || 0); if (!tanggal) { showToast('Tanggal harus diisi.'); return; }
if (nominal <= 0) { showToast('Nominal harus lebih dari 0.'); return; } showToast('Menyimpan ' + jenis + '...'); const payload = { tanggal: tanggal, jenis: jenis, qty: 1, satuan: 'PAKET', harga_satuan: nominal, nominal: nominal,
keterangan: 'Belum Dibagikan' // Menggunakan field keterangan sebagai status
}; const { error } = await db.from('waste_sales').insert([payload]); if (error) { showToast('Gagal menyimpan: ' + error.message); } else { showToast('Berhasil ditambahkan ke saldo!'); nomEl.value = '';
loadData('limbah'); // Reload data dari DB
} }; window.bagikanLimbah = async function(jenis) { if(!confirm(`Anda yakin ingin membagikan seluruh saldo ${jenis}? Tindakan ini akan mereset saldo menjadi Rp 0.`)) return; showToast('Memproses pembagian...'); const { error } = await db.from('waste_sales') .update({ keterangan: 'Sudah Dibagikan' })
.eq('jenis', jenis) .eq('keterangan', 'Belum Dibagikan'); if (error) { showToast('Gagal membagikan: ' + error.message); } else { showToast(`Saldo ${jenis} berhasil dibagikan & dinolkan!`); loadData('limbah'); } }; window.renderWasteSalesTable = function() { const tbody = $('wasteSalesTableBody');
if (!tbody) return; const list = [...(DB.wasteSales || [])].sort((a,b) => b.tanggal.localeCompare(a.tanggal)); if (!list.length) { tbody.innerHTML = '<tr><td colspan="5" class="center empty">Belum ada riwayat penjualan limbah.</td></tr>'; return; } tbody.innerHTML = list.map((item, i) => {
const isBelum = item.keterangan === 'Belum Dibagikan';
